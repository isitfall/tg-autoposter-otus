...
export const TelegramMessages = {
  ...
  channel: {
    ...
    channelsList: (channelsList: string) =>
      `<b>Ваши каналы:</b>\n\n${channelsList}`,
  },
  ...
  post: {
    ...
    publishError: (channelTitle: string, error: string) =>
      `<b>Ошибка публикации в канале ${channelTitle}:</b>\n${error}`,
    confirmPublish: (
      content: string,
      channelTitle: string,
      scheduledAt?: Date,
